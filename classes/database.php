...
class database{
    ...
    function updateUser($user_id, $firstname, $lastanme, $birthday, $sex) {
        try {
            $con = $this->opencon();
            $con->beginTransaction();
            $query = $con->prepare("UPDATE users SET user_firstname=?, user_lastname=?, user_birthday=?, user_sex=?
            WHERE user_id=?");
            $query->execute([$firstname, $lastanme, $birthday, $sex, $user_id]);
